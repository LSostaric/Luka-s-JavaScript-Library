...
function submitDataByHttpRequest(
	serverSideScript, callbackFunction, method, dataObject
) {
...
function submitDataByPostMethod(
	serverSideScript, callbackFunction, dataObject
) {
...
function submitDataByGetMethod(
	serverSideScript, callbackFunction, dataObject
) {
	...
	for(var i = 0; i < tokens.length; i++) {
		...
		if(i == 0) {
			...
			if(lastObject.length!==undefined) {
				...
				lastObject = filterElementsByParentNode(
					x,
					lastObject
				);
				...
				for(var j = 0; j<lastObject.length; j++) {
					...
					if(x.length !== undefined) {
						...
						for(var k = 0; k < x.length; k++) {
							...
							if(
								indexOfArrayValue(result, x[k]) == -1 && i == tokens.length - 1
							) {
...
function fetchPage(pageNumber, parentElement,
	headerClass, elementsPerPage, displayStyle,
	hiddenStyle
) {
...
function countDown(
	targetDate, targetId, classDays, classHours, classMinutes, classSeconds
) {
